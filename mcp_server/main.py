# generated by fastapi-codegen:
#   filename:  openapi.yaml
#   timestamp: 2025-06-28T09:25:48+00:00



import argparse
import json
import os
from typing import *
from typing import Optional, Union

from autogen.mcp.mcp_proxy import MCPProxy
from autogen.mcp.mcp_proxy.security import BaseSecurity
from fastapi import Query
from pydantic import constr

from models import AnalyticsItineraryPriceMetricsGetResponse, Error400, Error500

app = MCPProxy(
    description='Before using this API, we recommend you read our **[Authorization Guide](https://developers.amadeus.com/self-service/apis-docs/guides/authorization-262)** for more information on how to generate an access token.',
    title='Flight Price Analysis API',
    version='1.0.1',
    servers=[{'url': 'https://test.api.amadeus.com/v1'}],
)


@app.get('/analytics/itinerary-price-metrics', tags=['itinerary_pricing_analysis'])
def get_itinerary_price_metrics(
    origin_iata_code: constr(
        pattern=r'[a-zA-Z]{3}', min_length=3, max_length=3
    ) = Query(..., alias='originIataCode'),
    destination_iata_code: constr(
        pattern=r'[a-zA-Z]{3}', min_length=3, max_length=3
    ) = Query(..., alias='destinationIataCode'),
    departure_date: str = Query(..., alias='departureDate'),
    currency_code: Optional[
        constr(pattern=r'[a-zA-Z]{3}', min_length=3, max_length=3)
    ] = Query('EUR', alias='currencyCode'),
    one_way: Optional[bool] = Query('false', alias='oneWay'),
):
    """
    GET itinerary price metric
    """
    raise RuntimeError("Should be patched by MCPProxy and never executed")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MCP Server")
    parser.add_argument(
        "transport",
        choices=["stdio", "sse", "streamable-http"],
        help="Transport mode (stdio, sse or streamable-http)",
    )
    args = parser.parse_args()

    if "CONFIG_PATH" in os.environ:
        config_path = os.environ["CONFIG_PATH"]
        app.load_configuration(config_path)

    if "CONFIG" in os.environ:
        config = os.environ["CONFIG"]
        app.load_configuration_from_string(config)

    if "SECURITY" in os.environ:
        security_params = BaseSecurity.parse_security_parameters_from_env(
            os.environ,
        )

        app.set_security_params(security_params)

    mcp_settings = json.loads(os.environ.get("MCP_SETTINGS", "{}"))

    app.get_mcp(**mcp_settings).run(transport=args.transport)
